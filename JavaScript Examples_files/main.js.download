var TopNavBar = {
  ...
  pathfinderApiBaseUrl: 'https://pathfinder-api.kai.w3spaces.com',
...
TopNavBar.renderProfilePicture = function (profilePictureUrl) {
...
TopNavBar._applyUserSessionUiTweaks = function (prs) {
  ...
  if (loggedIn) {
    ...
    if (TopNavBar.profilePictureUrl === null) {
      ...
      Util.fetch({
        context: 'TopNavBar -> _applyUserSessionUiTweaks',
        method: 'GET',
        ...
        callback: function (reqRes) {
          if (
            reqRes.error.code === '0'
            && typeof reqRes.data.personal !== 'undefined'
            && typeof reqRes.data.personal.profilePicture !== 'undefined'
            && reqRes.data.personal.profilePicture
          ) {
            var profilePictureUrl = TopNavBar.altProfileBaseUrl + '/' + reqRes.data.personal.profilePicture;

            TopNavBar.renderProfilePicture(profilePictureUrl);
          } else {
